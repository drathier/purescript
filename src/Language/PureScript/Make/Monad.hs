...
readCborFile :: (MonadIO m, MonadError MultipleErrors m) => NFData a => Binary a => FilePath -> m (Maybe a)
readCborFile path =
...
writeCborFileIO path value = do
  ...
  BSL.writeFile path bslc
  -- Binary.encodeFile path (GZip.compressWith (GZip.defaultCompressParams GZip.bestSpeed) value)
